package shop.rns.kakaobroker.dlx;
    ...
    public boolean handleErrorProcessingMessage(Message message, Channel channel) {
        ...
        try {
            ...
            if (rabbitmqHeader.getFailedRetryCount() >= maxRetryCount) {
                ...
                rabbitTemplate.convertAndSend(DEAD_EXCHANGE_NAME, "kakao.dead." + brokerName, kakaoMessageResultDto);

                // 다른 중계사를 다 돌지 않았을 경우, KE 중계사 WAIT로 보내기
                ...
                channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
                rabbitTemplate.convertAndSend(DLX_EXCHANGE_NAME, KE_WAIT_ROUTING_KEY, message);
            }
